...
public class PlainTextSiteOutputHandler extends GsacOutputHandler {
    ...
    private void addHeader (PrintWriter pw) {
        pw.append(  "   GSAC data respository: site information in plain text. \n");
        pw.append(  "   Generated by the "+ getRepository().getRepositoryName()  + " on "+ myFormatDateTime(new Date()) + " \n"); 
        pw.append(  "   This format is only for a quick visual check of what is available.  Not intended for computer processing. \n");
        pw.append(  "   Empty times (no characters) may mean 'not removed' or 'no change.' ");
        pw.append(  "   This format may change without notice.'\n");
    }
    ...
    private void addSiteLocation(PrintWriter pw, GsacSite site)
        ...
        // enable print of GSAC's 'elevation' value if your locations have ellipsoid height
        pw.append(    " site ellipsoidal height:     " + elevationFormat.format(el.getElevation()) + "" + "\n");
        pw.append(    " site ellipsoid name           \n");  // LOOK need GSAC parameter for this item
        // OR enable this print, if you use elevation above a geoid, not ellipsoid height:
